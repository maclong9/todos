...
struct LayoutView: Element {
  let content: HTMLContentBuilder

  init(@HTMLBuilder content: @escaping HTMLContentBuilder) {
  ...
  var body: some HTML {
    BodyWrapper {
      Header {
        Stack(classes: ["content"]) {
          Link(to: "/", classes: ["title"]) {
            Text { "Todos" }
          }
          Navigation {
            Link(
              to: "https://github.com/maclong9/todos",
              newTab: true
            ) {
      ...
      Footer {
        Text {
          "Built with "
          Link(to: "https://github.com/maclong9/web-ui") { "WebUI" }
          " by "
          Link(to: "https://maclong.uk") { "Mac" }
        }
      }
    }
  }
