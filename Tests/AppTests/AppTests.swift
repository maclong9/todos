...
final class AppTests: XCTestCase {
  ...
  func createUser(_ user: CreateUserRequest, client: some TestClientProtocol)
    async throws
    -> CreateUserResponse
  {
    try await client.execute(
      ...
      body: JSONEncoder().encodeAsByteBuffer(
        user, allocator: ByteBufferAllocator())
    ) { response in
      XCTAssertEqual(response.status, .created)
      return try JSONDecoder().decode(
        CreateUserResponse.self, from: response.body)
    }
  }

  func login(
    username: String, password: String, client: some TestClientProtocol
  ) async throws
    -> String?
  {
    try await client.execute(
  ...
  func createTodo(
    _ todo: CreateTodoRequest, cookie: String? = nil,
    ...
    async throws -> Todo
  {
    ...
    return try await client.execute(
      ...
      headers: headers,
      body: JSONEncoder().encodeAsByteBuffer(
        todo, allocator: ByteBufferAllocator())
    ) { response in
      guard response.status == .created else {
        throw TestError.unexpectedStatus(response.status)
      }
  ...
  func getTodo(
    _ id: String, cookie: String? = nil, client: some TestClientProtocol
  ) async throws
    -> Todo?
  {
    try await client.execute(
  ...
  func editTodo(
    _ id: String, _ todo: EditTodoRequest, cookie: String? = nil,
    client: some TestClientProtocol
  ) async throws -> Todo? {
    var headers: HTTPFields = [.contentType: "application/json"]
    if let cookie = cookie {
  ...
  func testCreateUser() async throws {
    ...
    try await app.test(.router) { client in
      _ = try await self.createUser(
        .init(
          name: "Tom Jones", email: "t@jones.com",
          password: "password123"), client: client)
    }
  }
  ...
  func testGetTodo() async throws {
    ...
    try await app.test(.router) { client in
      ...
      let getTodo = try await self.getTodo(
        todo.id, cookie: cookie, client: client)
  ...
  func testEditTodo() async throws {
    ...
    try await app.test(.router) { client in
      ...
      _ = try await self.editTodo(
        todo.id, .init(title: "Written tests", completed: true),
        cookie: cookie, client: client)
      let editedTodo = try await self.getTodo(
        todo.id, cookie: cookie, client: client)
  ...
  func testUnauthorizedEditTodo() async throws {
    ...
    try await app.test(.router) { client in
      ...
      let cookie2 = try await self.login(
        username: "lu@lu.com", password: "admin", client: client)
      do {
        _ = try await self.editTodo(
          todo.id, .init(title: "Written tests", completed: true),
          cookie: cookie2, client: client)
      } catch TestError.unexpectedStatus(let status) {
