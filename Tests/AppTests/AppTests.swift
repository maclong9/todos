...
// TODO: Figure out if you can test frontend interactivity from here
...
final class AppTests: XCTestCase, @unchecked Sendable {
  ...
  func createUser(_ user: CreateUserRequest, client: some TestClientProtocol) async throws
    -> CreateUserResponse
  {
  ...
  func login(username: String, password: String, client: some TestClientProtocol) async throws
    -> String?
  {
    try await client.execute(
      uri: "/api/users/login",
  ...
  func urlEncodedLogin(username: String, password: String, client: some TestClientProtocol)
    async throws -> String?
  {
    try await client.execute(
      uri: "/login",
  ...
  func createTodo(_ todo: CreateTodoRequest, cookie: String? = nil, client: some TestClientProtocol)
    async throws -> Todo
  {
  ...
  func getTodo(_ id: String, cookie: String? = nil, client: some TestClientProtocol) async throws
    -> Todo?
  {
    return try await client.execute(
      uri: "/api/todos/\(id)",
  ...
  func editTodo(
    _ id: String, _ todo: EditTodoRequest, cookie: String? = nil, client: some TestClientProtocol
  ) async throws -> Todo? {
    var headers: HTTPFields = [.contentType: "application/json"]
    if let cookie = cookie {
      headers[.cookie] = cookie
  ...
  func testCreateUser() async throws {
    ...
    try await app.test(.router) { client in
      _ = try await self.createUser(
        .init(name: "Tom Jones", email: "t@jones.com", password: "password123"), client: client)
    }
  }
  ...
  func testURLEncodedLogin() async throws {
    ...
    try await app.test(.router) { client in
      ...
      let cookie = try await self.urlEncodedLogin(
        username: "t@jones.com", password: "password123", client: client)
  ...
  func testCreateTodo() async throws {
    ...
    try await app.test(.router) { client in
      ...
      let todo = try await self.createTodo(
        .init(title: "Write more tests"), cookie: cookie, client: client)
  ...
  func testEditTodo() async throws {
    ...
    try await app.test(.router) { client in
      ...
      _ = try await self.editTodo(
        todo.id, .init(title: "Written tests", completed: true), cookie: cookie, client: client)
  ...
  func testUnauthorizedEditTodo() async throws {
    ...
    try await app.test(.router) { client in
      _ = try await self.createUser(
        .init(name: "Tom Jones", email: "t@jones.com", password: "password123"), client: client)
      _ = try await self.createUser(
        .init(name: "Lulu", email: "lu@lu.com", password: "admin"), client: client)
      let cookie = try await self.login(
        username: "t@jones.com", password: "password123", client: client)
      let todo = try await self.createTodo(
        .init(title: "Write more tests"), cookie: cookie, client: client)
      ...
      do {
        _ = try await self.editTodo(
          todo.id, .init(title: "Written tests", completed: true), cookie: cookie2, client: client)
      } catch TestError.unexpectedStatus(let status) {
